...
private:
  ...
  static bool sameEndpoint(const Endpoint& configured,
                           const sockaddr *actual,
                           socklen_t actualLength)
  {
    const sockaddr *expected = reinterpret_cast<const sockaddr *>(&configured.address);
    if (actual == nullptr || expected->sa_family != actual->sa_family)
    {
      return false;
    }
    if (expected->sa_family == AF_INET &&
        configured.length == sizeof(sockaddr_in) && actualLength == sizeof(sockaddr_in))
    {
      const sockaddr_in *expected4 = reinterpret_cast<const sockaddr_in *>(expected);
      const sockaddr_in *actual4 = reinterpret_cast<const sockaddr_in *>(actual);
      return expected4->sin_port == actual4->sin_port &&
             std::memcmp(&expected4->sin_addr, &actual4->sin_addr, sizeof(in_addr)) == 0;
    }
    if (expected->sa_family == AF_INET6 &&
        configured.length == sizeof(sockaddr_in6) && actualLength == sizeof(sockaddr_in6))
    {
      const sockaddr_in6 *expected6 = reinterpret_cast<const sockaddr_in6 *>(expected);
      const sockaddr_in6 *actual6 = reinterpret_cast<const sockaddr_in6 *>(actual);
      return expected6->sin6_port == actual6->sin6_port &&
             expected6->sin6_scope_id == actual6->sin6_scope_id &&
             std::memcmp(&expected6->sin6_addr, &actual6->sin6_addr, sizeof(in6_addr)) == 0;
    }
    return false;
  }
...
public:
  ...
  bool set(const sockaddr *address, socklen_t length, bool freebind = false)
  {
    if (address == nullptr ||
    ...
    Endpoint& endpoint = address->sa_family == AF_INET ? ipv4 : ipv6;
    endpoint = {};
  ...
  const Endpoint *lookup(sa_family_t family) const
  {
    const Endpoint *endpoint = family == AF_INET
                                   ? &ipv4
                                   : family == AF_INET6 ? &ipv6 : nullptr;
    return endpoint && *endpoint ? endpoint : nullptr;
  }
  ...
  bool bind(int fd) const
  {
    ...
    const Endpoint *endpoint = lookup(socketAddress.ss_family);
    if (endpoint == nullptr)
    {
      return false;
    }
    ...
    return setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &enabled, sizeof(enabled)) == 0 &&
           ...
           ::bind(fd,
                  ...
                  endpoint->length) == 0 &&
           getsockname(fd, reinterpret_cast<sockaddr *>(&actual), &actualLength) == 0 &&

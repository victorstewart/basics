...
struct sherwood_v8_constants {
  constexpr static int8_t magic_for_empty = int8_t(0b1111'1111);
  constexpr static int8_t magic_for_reserved = int8_t(0b1111'1110);
  constexpr static int8_t bits_for_direct_hit = int8_t(0b1000'0000);
  constexpr static int8_t magic_for_direct_hit = int8_t(0b0000'0000);
  constexpr static int8_t magic_for_list_entry = int8_t(0b1000'0000);

  constexpr static int8_t bits_for_distance = int8_t(0b0111'1111);
  static inline int distance_from_metadata(int8_t metadata)
  {
    return metadata & bits_for_distance;
  }

  constexpr static int num_jump_distances = 126;
  ...
  constexpr static size_t jump_distances[num_jump_distances] {
      0,
      1,
      2,
      3,
      ...
      309'482'258'302'503,
      696'335'090'510'256,
      1'566'753'939'653'640,
      3'525'196'427'195'653,
      7'931'691'866'727'775,
      17'846'306'747'368'716,
      40'154'190'394'120'111,
      90'346'928'493'040'500,
      203'280'588'949'935'750,
      457'381'324'898'247'375,
      1'029'107'980'662'394'500,
      2'315'492'957'028'380'766,
      5'209'859'150'892'887'590,
  };
};
template <typename T>
...
struct sherwood_v8_block {
  ...
  {
    struct EmptyBlockStorage {
      alignas(sherwood_v8_block) unsigned char bytes[sizeof(sherwood_v8_block)];
    };

    static EmptyBlockStorage storage = [] {
      EmptyBlockStorage result {};
      std::fill_n(reinterpret_cast<int8_t *>(result.bytes), BlockSize, sherwood_v8_constants<>::magic_for_empty);
      return result;
    }();

    return reinterpret_cast<sherwood_v8_block *>(storage.bytes);
  }
...
public:
  ...
  size_t consumingNBytesOfMemory(void)
  {
    size_t nBuckets = bucket_count();
    size_t nBlocks = nBuckets / BlockSize;

    if (nBuckets % BlockSize)
    {
      ++nBlocks;
    }

    return calculate_memory_requirement(nBlocks);
  }
...
private:
  ...
  struct LinkedListIt {
    ...
    int8_t jump_index() const
    {
      ...
      int distance = Constants::distance_from_metadata(metadata());
      if (distance <= 0 || distance >= Constants::num_jump_distances)
      {
        return 0;
      }

      return static_cast<int8_t>(distance);
    }
  ...
  LinkedListIt find_parent_block(LinkedListIt child)
  {
    ...
    for (;;)
    {
      ...
      if (++hops >= static_cast<size_t>(Constants::num_jump_distances))
      {
        return parent_block;
      }
    }
...
struct AlignmentOr8Bytes<T, typename std::enable_if<alignof(T) >= 1>::type> {
  constexpr static size_t value = alignof(T);
};
template <typename... Args>
struct CalculateBytellBlockSize;
template <typename First, typename... More>
struct CalculateBytellBlockSize<First, More...> {
  constexpr static size_t this_value = AlignmentOr8Bytes<First>::value;
  constexpr static size_t base_value = CalculateBytellBlockSize<More...>::value;
...
public:
  ...
  bytell_hash_map(size_t sizeToReserve)
  {
    this->reserve(sizeToReserve);
  }

  using Table::emplace;
  std::pair<typename Table::iterator, bool> findOrConstruct(const K& key)
  {
    auto it = this->find(key);

    if (it == this->end())
    {
      return emplace(key, convertible_to_value());
    }
  ...
  V& atOrConstruct(const K& key)
  {
    return findOrConstruct(key).first->second;
  }

  V getIf(const K& key) requires (std::is_integral_v<V>)
  {
    auto it = this->find(key);
